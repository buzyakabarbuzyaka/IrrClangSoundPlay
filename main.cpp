...
int main(int argc, const char** argv)
{
	...
	irrklang::ISoundSource* ambient = engine->addSoundSourceFromFile("../media/music/getout.ogg");
	ambient->setDefaultVolume(0.01f);
	irrklang::ISoundSource* dieSnd = engine->addSoundSourceFromFile("../media/music/oof.wav");
	
	// To play a sound, we only to call play2D(). The second parameter
	// tells the engine to play it looped.

	// play some sound stream, looped
	irrklang::ISound* snd = engine->play2D(ambient, true);
	//engine->play2D("../media/music/getout.ogg", true);
